...
symboling = st.slider("Etat de route (risque relatif de la voiture)",min_value=-2,max_value=3,step=1)
doornumber = st.radio('Nombre de portes',(2,4))
...
compressionratio = st.slider("Taux de compression du moteur", min_value=6.0,max_value=24.0)
...
feature_values = pd.DataFrame({'symboling' : symboling, 'fueltype':fueltype, 'aspiration' :aspiration,'doornumber': doornumber, 'carbody': carbody,
                   'drivewheel': drivewheel, 'enginelocation' : enginelocation, 'wheelbase':wheelbase, 'carlength' :carlength,'carwidth': carwidth, 'carheight':carheight,'curbweight':curbweight,'enginetype':enginetype,'cylindernumber':cylindernumber,'enginesize':enginesize,'fuelsystem':fuelsystem,'boreratio':boreratio,'stroke':stroke,'compressionratio' : compressionratio,'horsepower' : horsepower,'marque' :marque, 'city L/100km':cityconso,'highway L/100km' :highwayconso},index=[0])
